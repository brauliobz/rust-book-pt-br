...
O Cargo é sistema de _build_ e gerenciador de pacotes do Rust. A maior parte
dos rustáceos usam essa ferramenta para controlar seus projetos Rust, pois
o Cargo automatiza muitas tarefas comuns, tais como compilar o código, baixar
as bibliotecas das quais seu código depende e compilar tais bibliotecas.
(Chamamos de *dependências* as bibliotecas das quais seu código depende)
...
Os programas mais simples em Rust, como o que nós fizemos até agora, não têm
nenhuma dependência. Então se tivermos que compilar o projeto “Olá, mundo!”
com o Cargo, ele somente utilizaria a parte do Cargo que faz a compilação
do seu código. Quando você começar a escrever programas Rust mais complexos,
você adicionará dependências e, se você começar um projeto usando o Cargo,
será bem mais fácil de adicionar tais dependências.
...
Como a maioria dos projetos Rust utiliza Cargo, o resto desse livro assume
que você também o está utilizando. O Cargo já vem pré-instalado com Rust se
você tiver usado um dos instaladores oficiais discutidos na seção
[Instalação”][installation]. Se você tiver instalado Rust de alguma outra
forma, verifique se o Cargo está instalando usando esse comando no seu
terminal:
...
Se ele retornar um número de versão, você tem o Cargo instalado. Se você ver
um erro, tal como `comando não encontrado`, procure na documentação do seu
método de instalação para descobrir como instalar o Cargo separadamente.
